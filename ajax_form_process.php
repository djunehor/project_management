...
if($mum2['senderID']!=$senderid){$dad = $mum2['senderID'];}else{$dad = $mum2['recipientID'];}
if($utype==2)
{$mum3 = mysqli_fetch_assoc(mysqli_query($con,"SELECT lastseen FROM $user_table WHERE email='$dad'"));}
elseif($utype==1) {$mum3 = mysqli_fetch_assoc(mysqli_query($con,"SELECT lastseen FROM $employee_table WHERE email='$dad'"));}
...
{
echo '<div ';
if($f['senderID']==$senderid) {	echo 'style="text-align:right;" class="alert alert-success"><button style="float:right;" class="btn btn-danger btn-icon rounded-circle mg-r-5 mg-b-10" onclick="del_message(this.value)" name="btnDelete" value="'.$f['ID'].'">Delete</button>';}
else{echo 'class="alert alert-danger">';}

if(!empty($f['message'])) {echo substr(html_entity_decode(htmlspecialchars_decode($f['message'])),3,-8);	}
if(!empty($f['attachment'])){echo '<a target="_blank" href="'.$f['attachment'].'"><img src="'.$f['attachment'].'" width="50px" height="50px">View/Download Image</a>';}	
...
case 'assign_task':
$array=array("projectid","milestoneid","taskid","email","comment","managername","managerid");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if(mysqli_num_rows(mysqli_query($con,"select * from $task_table where taskID='$taskid'"))!=1){
$error = 'Task Not Found!';
}
else if(mysqli_num_rows(mysqli_query($con,"select * from $project_table where projectID='$projectid'"))!=1){
$error = 'Project Not found!';
}
else if(mysqli_num_rows(mysqli_query($con,"select * from $milestone_table where milestoneID='$milestoneid'"))!=1){
$error = 'Milestone Not found!';
}
else if(mysqli_num_rows(mysqli_query($con,"select * from $employee_table where email='$email'"))!=1){
...
case 'task_com':
$array=array("managerid","projectid","milestoneid","taskid","tcomment","tcost","employeeid");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if(mysqli_num_rows(mysqli_query($con,"select * from $employee_table where email='$employeeid'"))!=1){
$error = 'Employee does not exist. Employee must register first!';
}
else
{
...
case 'new_task':
$array=array("title","desc","projectid","milestoneid","duration","depend","managerid");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if(mysqli_num_rows(mysqli_query($con,"select title from $task_table where projectID='$projectid' AND milestoneID='$milestoneid'"))>0){
...
case 'new_listing':
$array=array("title","info","city","state","country","info","price","ltype","atype","managerid","address");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if(mysqli_num_rows(mysqli_query($con,"select * from $apartment_table where name='$title'"))>0){
$error = 'Title Already Exists!';
}
elseif(count($_FILES['upload']['name'])<1) {$error = 'You must upload at least 1 photo!';}
elseif(count($_FILES['upload']['name'])>8) {$error = 'You can upload maximum of 8 photos!';}
else
{
...
$array=array("fullname","phone","managerID");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
case 'eprofile':
$array=array("fullname","phone","employeeID","website","jobdesc");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
case 'new_project':
$array=array("title","desc","startdate","enddate","budget","status");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if($enddate < strtotime('today') || $startdate < strtotime('today')){
$error = 'Date cannot be less than today!';
}
else if($enddate < $startdate){
$error = 'End Date must be greater than Start date';
}
else if($status==1 && $startdate>strtotime('today')){
$error = 'Start date must be today if project should start immediately';
}
else if(mysqli_num_rows(mysqli_query($con,"select title from $project_table where title='$title'"))>0){
...
case 'new_milestone':
$array=array("title","desc","startdate","projectid","managerid","tag");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if($startdate < strtotime('today')){
$error = 'Date cannot be less than today!';
}
else if(mysqli_num_rows(mysqli_query($con,"select title from $milestone_table where title='$title'"))>0){
$error = 'Title Already Exist';
}
else if(mysqli_num_rows(mysqli_query($con,"select * from $project_table where projectID='$projectid'"))!=1){
$error = 'Project Not found!';
}
else if(mysqli_num_rows(mysqli_query($con,"select startDate from $milestone_table where projectID='$projectid' and startDate='$startdate'"))>0){
...
$pword = md5($pasword);
	...
	ActivityLog($con,$detail,$_SESSION['managerID']);
			...
			if($login['LastLogin']<1) {$result = 'Login Successful. Redirecting to profile page in 3 seconds. <script>window.setTimeout(function(){ window.location = "'.$website_url.'/manager/EditProfile"; },3000)</script>';}
			else{$result = 'Login Successful. Redirecting in 5 seconds. <script>window.setTimeout(function(){ window.location = "'.$website_url.'/manager/"; },5000)</script>';}
	}
		if(isset($error)) { echo "Error: ".$error; } else{echo $result;}
	break;
...
$pword = md5($pasword);
	...
	ActivityLog($con,$detail,$_SESSION['employeeID'],$type=1);
			...
			if($login['lastLogin']<1) {$result = 'Login Successful. Redirecting to profile page in 3 seconds. <script>window.setTimeout(function(){ window.location = "'.$website_url.'/employee/EditProfile"; },3000)</script>';}
			else{$result = 'Login Successful. Redirecting in 5 seconds. <script>window.setTimeout(function(){ window.location = "'.$website_url.'/employee/"; },5000)</script>';}
	}
		if(isset($error)) { echo "Error: ".$error; } else{echo $result;}
	break;
...
$array=array("email","password","confpass");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
else if(mysqli_num_rows(mysqli_query($con,"select * from $employee_table where email='$email'"))>0){
$error = 'Email Already Exist';
}
else
{
...
case 'contact':
$array=array("email","message","website","name");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
...
case 'contact_agent':
$array=array("email","message","recipient","listingid");
foreach($_REQUEST as $key=>$value){
	if(in_array($key,$array)){
		$$key=addslashes($value);
	}
