...
<span style="float:left;color:black;">
	 ...
	 <?php }
if(mysqli_num_rows($ao)<1) {echo 'No contractor assigned yet. <a href="AssignContractor?id='.$t1['taskID'].'">Assign Task</a>';}
	 ?></div> <hr>	
	 <?php }
if(mysqli_num_rows($ap)<1) {echo 'No task created yet. <a href="NewTask?id='.$m1['milestoneID'].'">Create New Task</a>';}
	 ?>	</div>	
	 <?php }
if(mysqli_num_rows($as)<1) {echo 'No milestone created yet. <a href="NewMilstone?id='.$project['projectID'].'">Create New Task</a>';}
	 ?></div>
</div>
      <!-- sl-pagebody -->
