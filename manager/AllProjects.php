...
ON $project_table.projectStatus=$status_table.statusID WHERE managerID='$managerID'");
                    while ($v = mysqli_fetch_assoc($q8)) {
                        ...
                        <tr class="<?php echo $v['projectID']; ?>">
                            <td><a href="ViewProject?id=<?php echo $v['projectID']; ?>"><?php echo $v['title']; ?></a>
                            </td>
                            ...
                            <?php if ($v['projectStatus'] == 0) { ?>
                                <td>
                                    <button onclick="start_project(this.value)" id="btnStart" type="submit"
                                            value="<?php echo $v['projectID']; ?>" class="btn btn-success">Mark As
                                        started
                                    </button>
                                </td> <?php } ?>
                            <td>
                                <form action="EditProject?id=<?php echo $v['projectID']; ?>">
                                    <button id="btnEdit" type="submit" class="btn btn-info">Edit</button>
                                </form>
                            </td>
                            <td>
                                <button onclick="del_project(this.value)" id="btnDelete" type="submit"
                                        value="<?php echo $v['projectID']; ?>" class="btn btn-danger">Delete
                                </button>
                            </td>
                        </tr>
                        <?php
