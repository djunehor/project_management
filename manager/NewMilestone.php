...
<form id="new_mile" role="form">
         ...
         <!--     <p>Try the keyboard navigation by clicking arrow left or right!</p> -->
                ...
                <input onblur="check_mtitle(this.value)" id="mile_name" class="form-control" name="title" placeholder="Enter title" type="text" required>
   <div class=" alert alert-danger pname_error_show" style="display:none"></div>
    <br>
	 ...
	 echo '<option value="'.$ol['tagID'].'">'.$ol['detail'].'</option>';
	  ...
	  ?>
		  ...
		  <button id="btnSubmit" type="submit" class="btn btn-info btn-block">Add MileStone</button></form>
	...
	$("#new_mile").on('submit',(function(e) {
		...
		$.ajax({
			url: "<?php echo $website_url; ?>/includes/form_process?value=new_milestone",
			type: "POST",
...
function check_mtitle(value){
	$.post("<?php echo $website_url; ?>/includes/ajax_check_input?value=mtitle",{title:value},function(data){
		if(data.length != 0){
