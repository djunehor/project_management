...
class validateInput
{
    ...
    */
    public function password($value, $min = 6, $max = 20)
    {
        if (!is_int($min) || !is_int($max)) {
            $output = 'Invalid arguments supplied!';
        } elseif (strlen($value) < $min) {
            $output = "Password too short! Minimum $min characters.";
        } elseif (strlen($value) > $max) {
            $output = "Password too long! Maximum $max characters.";
    ...
    public function email($email, $dns_check = true)
    {
        ...
        $ipcheck = preg_replace(['/^\[ipv6\:/i', '/^\[/', '/\]$/'], '', $domain);
    ...
    /*Method to validate numbers
        ...
        *$type is optional
        */
    public function number($value, $type = 0)
    {
        //check if is number
        switch ($type) {
...
class validateFile
{
    public function photo($file, $max = 0.2)
    {
